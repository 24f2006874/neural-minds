...
/** Local section heading (page-level heading is the shared SectionHeading). */
function BlockHeading({ index, title, sub }: { index: string; title: string; sub?: string }) {
  return (
    <div className="mb-5">
...
/** Dark recharts tooltip shared by all three charts. */
function DarkTip({
  active,
  payload,
  label,
    ...
    <div className="rounded-lg border border-[#22D3EE]/25 bg-[#081120]/95 px-3 py-2 text-xs shadow-[0_8px_24px_rgba(0,0,0,0.55)]">
      {label !== undefined && (
        <div className="font-display font-semibold text-[#E6F1FF]">
          {prefix ? `${prefix} ` : ""}
          {typeof label === "number" ? label.toFixed(labelDecimals) : label}
        </div>
      )}
      <div className="mt-1 space-y-0.5">
        {payload.map((entry, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full" style={{ background: entry.color ?? MUTED }} />
